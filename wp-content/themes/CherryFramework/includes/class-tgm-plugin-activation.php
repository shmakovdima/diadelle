			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . theme_locals("return") . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' .theme_locals("dashboard"). '">'.theme_locals("dashboard").'</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( theme_locals("following_plugin"), '<strong>' . $plugin['name'] . '</strong>' );
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, CURRENT_THEME ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */
					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, CURRENT_THEME ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, CURRENT_THEME ) . '</a>'  : '';
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' .theme_locals("dismiss"). '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = theme_locals("external_link");
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = theme_locals("private_repository");
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = theme_locals("pre_packaged");
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = theme_locals("wordpress_repository");
				}

				$table_data[$i]['type'] = $plugin['required'] ? theme_locals("required") : theme_locals("recommended");

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', theme_locals("not_installed") );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', theme_locals("installed_but") );
		...
		public function no_items() {
			...
			printf( theme_locals("no_plugins")."<a href='%1$s' title='".theme_locals("r_dashboard")."'>".theme_locals("r_dashboard")."</a>" , admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => theme_locals("plugin_"),
				'source' => theme_locals("source"),
				'type'   => theme_locals("type"),
				'status' => theme_locals("status")
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => theme_locals("install"),
				'tgmpa-bulk-activate' => theme_locals("activate"),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n(theme_locals("following_plugin"), theme_locals("following_plugins"), $count, CURRENT_THEME ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = theme_locals("no_package");
				$this->strings['downloading_package'] = theme_locals("downloading_package");
				$this->strings['unpack_package']      = theme_locals("unpack_package");
				$this->strings['installing_package']  = theme_locals("installing_package");
				$this->strings['process_failed']      = theme_locals("process_failed");
				$this->strings['process_success']     = theme_locals("process_success");
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = theme_locals("activation_failed");
				$this->strings['activation_success'] = theme_locals("activation_success");
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = theme_locals("");
					$this->upgrader->strings['skin_update_successful']    = theme_locals("skin_update_successful") . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . theme_locals("skin_update_successful_2") . '</span><span class="hidden">' . theme_locals("skin_update_successful_3") . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = theme_locals("skin_upgrade_end");
					$this->upgrader->strings['skin_before_update_header'] = theme_locals("skin_before_update_header");
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = theme_locals("skin_upgrade_start_2");
					$this->upgrader->strings['skin_update_failed_error']  = theme_locals("skin_update_failed_error");
					$this->upgrader->strings['skin_update_failed']        = theme_locals("skin_update_failed");
					$this->upgrader->strings['skin_update_successful']    = theme_locals("skin_update_successful_4") . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . theme_locals("skin_update_successful_2"). '</span><span class="hidden">' . theme_locals("skin_update_successful_3") . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = theme_locals("skin_upgrade_end_2");
					$this->upgrader->strings['skin_before_update_header'] = theme_locals("skin_before_update_header_2");
				}
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . theme_locals("dashboard") . '">' . theme_locals("dashboard") . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
